...
RSpec.describe 'site layout', type: :system do
  ...
  context 'Access to root_path when not logged in' do
    it 'has the correct header links' do
      expect(page).to have_link 'タスク管理アプリ', href: tasks_path
      expect(page).to have_link 'タスク一覧',      href: tasks_path
  ...
  context 'Access to root_path when logged in' do
    ...
    it 'has the correct header links' do
      expect(page).to_not have_link 'サインアップ', href: new_user_registration_path
      expect(page).to_not have_link 'ログイン',     href: new_user_session_path
      expect(page).to have_link 'マイタスク',       href: myself_tasks_path 
      expect(page).to have_link 'ユーザ設定',       href: edit_user_registration_path
